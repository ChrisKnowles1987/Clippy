...
@onready var storage_value_label: Label = $StorageValueLabel

func update_values(power: float, compute: float, storage: float):
	power_value_label.text = str(snapped(power, 0.1))
	compute_value_label.text = str(snapped(compute, 0.1))
	storage_value_label.text = str(snapped(storage, 0.1))
